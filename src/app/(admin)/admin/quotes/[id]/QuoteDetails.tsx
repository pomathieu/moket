// app/admin/quotes/[id]/QuoteDetails.tsx
import Link from 'next/link';
import { revalidateTag } from 'next/cache';

import { supabaseAdmin } from '@/lib/supabase/admin';
import { getQuoteCached, quoteTags } from '@/lib/quotes/cached';
...
export default async function QuoteDetails({ id }: { id: string }) {
  const { quote, error } = await getQuoteCached(id);
    ...
    return (
      <Card>
        ...
        <CardContent className="text-sm text-zinc-600">
          {error?.message ?? 'Aucun devis avec cet id.'}
          <div className="mt-4">
            <Button
              asChild
              variant="secondary">
  ...
  // ✅ Server Action robuste + invalidation tags
  async function setStatus(formData: FormData) {
    ...
    const { data, error } = await supabaseAdmin
      ...
      .update({
        ...
        updated_at: now, // garde si tu gères updated_at côté app
      })
      .eq('id', id)
      .eq('updated_at', expectedUpdatedAt) // ✅ concurrency guard
      .select('id')
      .maybeSingle();
    ...
    // Si data === null => conflit (quelqu'un a modifié)
    // Ici on invalide quand même, l'UI se remettra au bon état.
    revalidateTag(quoteTags.quote(id), 'default');
    revalidateTag(quoteTags.quoteEvents(id), 'default');
    revalidateTag(quoteTags.quotesList(), 'default');
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        ...
        {/* Col droite : l'historique est streamé depuis page.tsx */}
        <div className="space-y-4">
          {/* QuoteEvents va venir ici via page.tsx */}
          {/* Meta : on le garde ici, puisqu'on a déjà quote */}
          <Card>
            <CardHeader className="py-4">
